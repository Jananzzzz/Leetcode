...
# brute force method with long runtime
class Solution:
    def constructRectangle(self, area: int) -> list[int]:
        min_gap = [area, 1]
        for i in range(1, area//2 + 1):
            if (area/i == int(area/i)) and (abs(i - area/i) < abs(min_gap[0] - min_gap[1])):
                min_gap = [i, int(area/i)]
        return min_gap if min_gap[0] >= min_gap[1] else [min_gap[1], min_gap[0]]
    
class Solution:
    def constructRectangle(self, area: int) -> list[int]:
        res = [area, 1]
        min_gap = area - 1
        for i in range(int(area ** 0.5), area):
            if area/i == int(area/i) and abs(i - area/i) < min_gap:
                min_gap = abs(i - area/i)
                res = [i, int(area/i)]
...
print(Solution.constructRectangle(Solution(), 6))
print(Solution.constructRectangle(Solution(), 16))
print(Solution.constructRectangle(Solution(), 1))
print(Solution.constructRectangle(Solution(), 37))
print(Solution.constructRectangle(Solution(), 122122))
print(Solution.constructRectangle(Solution(), 4))