...
class Solution:
    def matrixReshape(self, mat: list[list[int]], r: int, c: int) -> list[list[int]]:
        ...
        res = []
        for i in mat:
            all_element.extend(i)
        if len(all_element) == r * c:
            for i in range(r):
                res.append(all_element[:c])
                all_element = all_element[c:]
            return res
        else:
            return mat

print(Solution.matrixReshape(Solution(), mat = [[1,2],[3,4]], r = 2, c = 4))
print(Solution.matrixReshape(Solution(), [[1,2],[3,4]], r = 1, c = 4))
